...
@section('main')
    ...
    <input type="hidden" id="doctor-user-id" value="{{ $id }}" />
    <input type="hidden" id="doctor-availability" value="{{ json_encode($user->availability) }}" />
    <input type="hidden" id="input-step" value="{{ $step }}" />
    ...
    <script>
        ...
        const step = parseInt(document.getElementById("input-step").value || "1");
    </script>
    ...
    <div class="container my-3">
        ...
        @switch($step)
            @case(1)
                    ...
                    <div id="servicesContainer" class="row g-3">
                        @foreach ($user->services as $service)
                            <div class="col-12 col-md-4">
                                <div class="service-card" onclick="onClickService(event);">
                                    <div class="service-details">
                                        <span class="service-name">{{ $service["name"] }}</span>
                                        <span class="service-price"><span>{{ $service["price"] }}</span> {{ config("config.currency") }}</span>
                                    </div>
                ...
                <script>
                    function onClickService(event) {
                        ...
                        const checkmark = node.querySelector(".checkmark-icon");
                    ...
                    function goNext(event) {
                        ...
                        const selectedServices = document.querySelectorAll(".service-card.active");
                        for (let a = 0; a < selectedServices.length; a++) {
                            const serviceName = selectedServices[a].querySelector(".service-name").innerHTML || "";
                            const servicePrice = selectedServices[a].querySelector(".service-price span").innerHTML || "0";
                        ...
                        localStorage.setItem("selected_service", JSON.stringify(selectedServicesArr));
            ...
            @case(2)
                <div class="card p-5 mt-5 custom-card-container">
                    ...
                    <div class="d-flex gap-3 justify-content-evenly">
                        ...
                        <div class="service-card appointment-type" data-type="video">
                            <div class="service-details appointment-type-detail"
                                onclick="onclickAppointmentType('video');">
                        ...
                        <div class="service-card appointment-type" data-type="audio">
                            <div class="service-details appointment-type-detail"
                                onclick="onclickAppointmentType('audio');">
                                <svg width="25" height="24" viewBox="0 0 25 24" fill="none"
                                    xmlns="http://www.w3.org/2000/svg">
                                    <path
                ...
                <script>
                    ...
                    function onclickAppointmentType(type = "") {
                        localStorage.setItem("selected_appointment_type", type);
                        // window.location.href = baseUrl + "/doctors/" + doctorUserId + "/schedules/update?step=3";
                    }
                </script>
            @break
            ...
            @case(3)
                <div class="card p-5 mt-5 custom-card-container">
                    <div class="row g-4">
                        <div class="col-12">
                            ...
                            <div class="row g-2 mt-2 time-slots-container mb-5"
                                id="days-time-slots">
                                @foreach ($days as $day)
                                    <div class="col-auto">
                                        <div class="time-slot-card"
                                            onclick="onchangeDay(event, '{{ $day }}');">
                                            {{ $day }}
                                        </div>
                                    </div>
                                @endforeach
                ...
                <script>
                    function onchangeDay(event, value) {
                        const node = event.currentTarget;
                        document.getElementById("input-day").value = value || "";

                        for (let a = 0; a < doctorAvailability.length; a++) {
                            if (doctorAvailability[a].day == value) {
                                const from = doctorAvailability[a].from || "";
                                const to = doctorAvailability[a].to || "";
                                const difference = doctorAvailability[a].difference || 0;

                                let html = "";
                                const timeSlots = generateTimeSlots(from, to, difference);
                                for (let b = 0; b < timeSlots.length; b++) {
                                    html += `<div class="col-auto">
                                        <div class="time-slot-card" onclick="onselectTimeSlot(event, '` + value + `', '` + timeSlots[b] + `');">
                                            ` + timeSlots[b] + `
                                        </div>
                                    </div>`;
                                }
                        ...
                        document.querySelectorAll("#days-time-slots .time-slot-card").forEach(function (n) {
                            n.classList.remove("active");
                        });
                        node.classList.toggle("active");
                    }

                    function onselectTimeSlot(event, day, time) {
                        document.querySelectorAll("#time-slots-container .time-slot-card").forEach(function (n) {
                            n.classList.remove("active");
                        });
            ...
            @case(4)
                    ...
                    <form onsubmit="return false;" enctype="multipart/form-data"
                        ...
                        <div class="row g-3">
                            ...
                            <div class="col-md-4">
                                ...
                                <input type="file" multiple name="attachments[]" class="custom-input-group form-control" id="attachment" />
                            ...
                            <div class="col-12 mt-4">
                                ...
                                <textarea class="form-control" name="reason_for_visit" id="reasonForVisit" rows="4"></textarea>
                            </div>
                        </div>
                    </form>
                </div>
                ...
                <script>
                    ...
                    async function saveAppointmentBasicInfo(node = null) {
                        const form = document.getElementById("form-basic-info");

                        try {
                            const formData = new FormData(form);
                            formData.append("doctor_user_id", doctorUserId);
                            formData.append("slot", localStorage.getItem("selected_slot"));
                            formData.append("service", localStorage.getItem("selected_service"));
                            formData.append("appointment_type", localStorage.getItem("selected_appointment_type"));

                            node?.setAttribute("disabled", "disabled");

                            const response = await axios.post(
                                baseUrl + "/appointments/basic-info",
                                formData
                            );
                            ...
                            if (response.data.status == "success") {
                              ...
                              localStorage.setItem("appointment_id", id);
                                ...
                                if (node == null)
                                    ...
                                    window.location.href = baseUrl + "/doctors/" + doctorUserId + "/schedules/update?step=" + (step + 1);
                            } else {
                    ...
                    async function goNext(event) {
                        ...
                        if (user == null) {
                            document.getElementById("phone-otp").innerHTML = form.phone.value || "";
                            
                            swal.fire("Phone OTP", "An OTP has been sent to your mobile number. Kindly enter to login and continue.", "success")
                                .then(function () {
                            ...
                            saveAppointmentBasicInfo(node);
                        }
                    }

                    async function onenteredOTP(event) {
                        const node = event.currentTarget;
                  ...
                  <div class="modal-dialog">
                    <div class="modal-content">
                      <div class="modal-header">
                        <h1 class="modal-title fs-5">Enter OTP sent to <span id="phone-otp"></span></h1>
                      ...
                      <div class="modal-footer">
                        ...
                        <button type="button" class="btn btn-primary" onclick="onenteredOTP(event);">Login</button>
                      </div>
                    </div>
                  </div>
                </div>
                        ...
                        <div class="offset-lg-3 col-lg-6">
                            <div class="card p-5 booking-info-card custom-card-container">
                                ...
                                <div class="booking-details-item mt-3">
                                    ...
                                    <p class="mb-0" id="appointment-detail"></p>
                                </div>
                                ...
                                <div class="mt-4">
                                    <button type="button" class="btn btn-primary btn-lg w-100 d-flex justify-content-between"
                                        ...
                                        Total
                                        <span class="ms-2" id="text-total"></span>
                                    </button>
                                </div>

                                <div id="stripe-card-element" class="mt-5"></div>
                            </div>
                        </div>
                    </div>
                </div>
                    ...
                    async function fetchAppointmentDetails() {
                        try {
                          ...
                          if (response.data.status == "success") {
                            ...
                            if (appointment != null) {
                                let selectedSlot = JSON.parse(localStorage.getItem("selected_slot") || "{}") || {};
                                document.getElementById("booking-date-time").innerHTML = selectedSlot.day + " at " + selectedSlot.time;

                                let selectedServices = JSON.parse(localStorage.getItem("selected_service") || "[]") || [];

                                let html = "";
                                let paymentHtml = "";
                                total = 0;

                                html += (appointment.doctor?.name || "");
                                let fee = parseFloat(appointment.doctor?.fee || "0");

                                paymentHtml += `<div class="d-flex justify-content-between my-2" style="font-weight: 500">
                                ...
                                for (let a = 0; a < selectedServices.length; a++) {
                                    html += " - " + selectedServices[a].name;
                                    let price = parseFloat(selectedServices[a].price || "0");
                                    total += price;

                                    paymentHtml += `<div class="d-flex justify-content-between my-2" style="font-weight: 500">
                                        <span>` + selectedServices[a].name + `</span>
                                        <span>
                                            ` + price + ` PKR
                                        </span>
                                    </div>`;
                                ...
                                html += " - Via " + localStorage.getItem("selected_appointment_type") + " call";
                                document.getElementById("appointment-detail").innerHTML = html;

                                let discount = parseFloat(appointment.doctor?.discount || "0");
                                total -= discount;

                                paymentHtml += `<div class="d-flex justify-content-between my-2">
                                    <span>Discount</span>
                                    <strong class="text-danger">-` + discount + ` PKR</strong>
                                </div>`;

                                document.getElementById("payment-details").innerHTML = paymentHtml;

                                document.getElementById("text-total").innerHTML = total + " PKR";
            ...
            @case(6)
                    ...
                    <div class="row g-4 justify-content-center">
                        <div class="col-lg-6">
                            <div class="card p-4 custom-card-container" style="display: flex; align-items: center; gap: 20px;">
                                <div class="" style="border: 1px solid #E6E8EE; border-radius: 10px;">
                                    ...
                                    <p class="mb-4 px-4 py-2" id="booking-details">
                                        <!--Your <span class="span-light-blue special-span span-bold">Request</span> for
                                        <span
                                            class="special-span span-bold span-underline">Regular Check-up</span>
                                            for <span
                                ...
                                <div class="d-flex px-4 py-2 justify-content-between align-items-center"
                                    ...
                                    <a href="tel:{{ fetch_setting('emergency_number') }}" class="btn d-flex align-items-center gap-2"
                        ...
                        <div class="col-lg-4">
                            <div
                                ...
                                <p class="booking-number-badge mb-3" id="appointment-number"></p>
                ...
                <script>
                        ...
                        try {
                          ...
                          if (response.data.status == "success") {
                            ...
                            if (appointment != null) {
                                ...
                                let html = "";
                                html += `Your <span class="span-light-blue special-span span-bold">Request</span> for
                                    <span class="special-span span-bold span-underline">Regular Check-up</span>`;

                                for (let a = 0; a < selectedServices.length; a++) {
                                    html += `&nbsp;<span class="special-span span-bold span-underline">` + selectedServices[a].name + `</span>`;
                                }

                                html += ` with
                                    <span class="special-span span-bold">
                                        <img src="` + (appointment.doctor?.profile_image || "") + `" style="object-fit: cover"
                                            width="25" height="25" alt="` + (appointment.doctor?.name || "") + `" class="rounded-circle me-1"
                                            onerror="this.remove();" />
                                        ` + (appointment.doctor?.name || "") + `
                                    </span> on

                                    <span class="special-span span-bold span-light-purple">
                                        <svg width="17" height="17" viewBox="0 0 17 17" fill="none"
                                            xmlns="http://www.w3.org/2000/svg">
                                            <path
                                                d="M0 14.2417C0 15.6158 1.05083 16.6667 2.425 16.6667H13.7417C15.1158 16.6667 16.1667 15.6158 16.1667 14.2417V7.775H0V14.2417ZM13.7417 2.11667H12.125V1.30833C12.125 0.823333 11.8017 0.5 11.3167 0.5C10.8317 0.5 10.5083 0.823333 10.5083 1.30833V2.11667H5.65833V1.30833C5.65833 0.823333 5.335 0.5 4.85 0.5C4.365 0.5 4.04167 0.823333 4.04167 1.30833V2.11667H2.425C1.05083 2.11667 0 3.1675 0 4.54167V6.15833H16.1667V4.54167C16.1667 3.1675 15.1158 2.11667 13.7417 2.11667Z"
                                        ...
                                        ` + selectedSlot.day + `
                                    </span> &nbsp;at <span class="special-span span-bold span-green">
                    ...
                    document.addEventListener('DOMContentLoaded', function () {
                        fetchAppointmentDetails();
                        document.getElementById("appointment-number").innerHTML = appointmentId;
                    });
                </script>
            @break
        ...
        <div class="mt-5 mb-5" style="text-align: right;">
            @if ($step > 1 && $step != 6)
                <a href="{{ url('/doctors/' . $id . '/schedules/update?step=' . ($step - 1)) }}" class="btn no-border white bold"
                    style="background-color: darkgray;">Back</a>
            @endif

            @if ($step < 6)
                <button type="button" href="{{ url('/doctors/' . $id . '/schedules/update?step=' . ($step + 1)) }}"
                    class="btn bg-primary-gradient no-border white bold"
                    style="background-color: darkgray;"
                    onclick="onclickNext(event);">Next</button>
            @endif
        </div>
