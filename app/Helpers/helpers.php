...
function set_timezone()
{
    ...
    if (empty($time_zone) && request()->expectsJson())
    {
        $time_zone = session(config("config.session_timezone_key")) ?? "";
    }
    if (!empty($time_zone) && in_array($time_zone, timezone_identifiers_list()))
    {
        date_default_timezone_set($time_zone);
    }
}

function get_absolute_path($value)
{
    if ($value && Storage::exists("public/" . $value))
    {
